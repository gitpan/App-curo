package App::curo::new::task; our $VERSION = '0.01_01';
use strict;
use warnings;
use App::curo::Util;
use Term::Prompt;
...
sub run {
    ...
    print "New task: $opt->{task_id} <$opt->{thread_uuid}>\n"
      if $db->insert_task($opt);
}
...
App::curo::new::task - insert a new task into the database
